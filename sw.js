...
const VERSION = 'share-1';
const SHELL_CACHE = 'libreroshare-shell-' + VERSION;
const COVER_CACHE = 'libreroshare-covers-' + VERSION;
...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    caches.keys().then((keys) =>
      Promise.all(
        keys.filter((k) => k.startsWith('libreroshare-') && k !== SHELL_CACHE && k !== COVER_CACHE)
